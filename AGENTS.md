...
- Releases: pushing a `vX.Y.Z` tag runs `.github/workflows/release.yml`, which builds an Apple Silicon (arm64) `Netpeek.app`, zips it, and attaches the zip + its SHA-256 to a **draft** GitHub Release - publishing is a manual, separate step. The app is ad-hoc signed only (no paid Apple Developer account), so it fails Gatekeeper on first launch; see the README's "Download and install" section for the user-facing fix.
